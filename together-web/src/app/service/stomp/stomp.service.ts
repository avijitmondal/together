...
export class STOMPService {
  ...
  public configure(config?: StompConfig): void {
    ...
    this.client = Stomp.client(`${scheme}://${this.config.host}:${this.config.port}/together/websocket`);
  ...
  public publish(message?: string): void {
    ...
    for (const t of this.config.publish) {
      console.log('Publish : ', t, ' Message : ', message);
      this.client.send('/app' + t, {}, message);
    }
  }
